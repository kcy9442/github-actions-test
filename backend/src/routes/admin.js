...
router.post('/products', upload.single('image'), asyncHandler(async (req, res) => {
  ...
  // 관리자 CRUD 요청은 API Gateway의 짧은 요청 제한 안에 끝나야 한다. Bedrock 번역/임베딩을
  // 여기서 동기로 기다리면 쿼터 재시도 중 30초를 넘겨 DynamoDB 저장 전 요청이 실패한다.
  // AI 보강은 시딩/별도 배치 작업의 책임으로 두고, 등록은 원문만 즉시 저장한다.
  const product = {
...
router.put('/products/:itemId', upload.single('image'), asyncHandler(async (req, res) => {
  ...
  // 원문이 바뀌면 과거 번역/임베딩이 새 상품 문구와 맞지 않는다. 별도 AI 보강 작업이
  // 생성할 때까지 클라이언트는 원문으로 안전하게 폴백한다.
